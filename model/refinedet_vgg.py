...
class RefineSSD(nn.Module):
    ...
    def __init__(self, size, num_classes, use_refine=False, phase='train', dropout=1.0):
        ...
        if self.use_refine:
            ...
            self.arm_conf = nn.ModuleList([nn.Conv2d(512, self.box_num *2, kernel_size=3, stride=1, padding=1),
                                           nn.Conv2d(512, self.box_num *2, kernel_size=3, stride=1, padding=1),
                                           nn.Conv2d(1024,self.box_num *2, kernel_size=3, stride=1, padding=1),
                                           nn.Conv2d(512, self.box_num *2, kernel_size=3, stride=1, padding=1),
                                           ])
